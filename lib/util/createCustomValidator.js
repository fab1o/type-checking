...
function createCustomValidator(type) {
  ...
  if (type.expectArgs) {
    var validateWithArguments = function validateWithArguments() {
      ...
      return _validateCreator.customValidateCreator.apply(void 0, [type, null, validator, message].concat(args));
    };

    return validateWithArguments.bind(type);
  }

  return (0, _validateCreator.customValidateCreator)(type, null, validator, message);
}